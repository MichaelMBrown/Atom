...
public class report implements CommandExecutor {
    ...
    public void notifyStaff(Player sender, Player target, String[] args) {
        if (args[1].equalsIgnoreCase("KA")) {
            staffUtils.scNotify("Console", staffUtils.retrievePlayerColour(sender) + sender.getName() + ChatColor.YELLOW + " has reported " + staffUtils.retrievePlayerColour(target) + target.getName() + ChatColor.YELLOW + " for " + ChatColor.RED + "KillAura" + ChatColor.YELLOW + ".");
        } else
            staffUtils.scNotify("Console", staffUtils.retrievePlayerColour(sender) + sender.getName() + ChatColor.YELLOW + " has reported " + staffUtils.retrievePlayerColour(target) + target.getName() + ChatColor.YELLOW + " for " + ChatColor.RED + args[1].toLowerCase() + ChatColor.YELLOW + ".");
    }
}