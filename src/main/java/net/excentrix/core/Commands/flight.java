...
//                                sender.sendMessage(ChatColor.GREEN + "You turned flight on for " + ChatColor.YELLOW + target.getName() + ChatColor.GREEN + "!");
								staffUtils.informativeMessage((Player) sender, "You turned flight on for " + staffUtils.getPlayerColor(target) + target.getName() + "&a!");
								if (!target.isOnGround() || !target.isSwimming()) {
									target.setAllowFlight(true);
									target.setFlying(true);
								} else target.setAllowFlight(true);
							} else if (args[1].equalsIgnoreCase("off")) {
								staffUtils.informativeMessage((Player) sender, "You turned flight off for " + staffUtils.getPlayerColor(target) + target.getName() + "&a!");
						...
						if (((Player) sender).getAllowFlight()) {
							staffUtils.informativeMessage((Player) sender, "You turned flight off for " + staffUtils.getPlayerColor((Player) sender) + sender.getName() + "&a!");
							...
							staffUtils.informativeMessage((Player) sender, "You turned flight on for " + staffUtils.getPlayerColor((Player) sender) + sender.getName() + "&a!");
						}
					} else
