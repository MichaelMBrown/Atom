...
//                                sender.sendMessage(ChatColor.GREEN + "You turned flight on for " + ChatColor.YELLOW + target.getName() + ChatColor.GREEN + "!");
                                staffUtils.informativeMessage((Player) sender, "You turned flight on for " + Core.playerColour + target.getName() + "&7!");
                                if (!target.isOnGround() || !target.isSwimming()) {
                                    target.setAllowFlight(true);
                                    target.setFlying(true);
                                } else target.setAllowFlight(true);
                            } else if (args[1].equalsIgnoreCase("off")) {
                                staffUtils.informativeMessage((Player) sender, "You turned flight off for " + Core.playerColour + target.getName() + "&7!");
                        ...
                        if (((Player) sender).getAllowFlight()) {
                            staffUtils.informativeMessage((Player) sender, "You turned flight off for " + Core.playerColour + sender.getName() + "&7!");
                            ...
                            staffUtils.informativeMessage((Player) sender, "You turned flight on for " + Core.playerColour + sender.getName() + "&7!");
                        }
                    } else
