	...
	public static void messagePlayer(Player sender, Player receipt, String message) {
		for (final Player p : Bukkit.getOnlinePlayers()) {
			if (Core.nowSpying.contains(p) && p != sender && p != receipt) {
				staffUtils.informativeMessage(p, "&9(" + staffUtils.getPlayerColor(sender) + sender.getName() + "&9 to " + staffUtils.getPlayerColor(receipt) + receipt.getName() + "&9) " + message);
			}
		}
		plugin.getLogger().info(ChatColor.translateAlternateColorCodes('&', "&c" + sender.getName() + " &f-> &c" + receipt.getName() + " &f" + message));
		staffUtils.informativeMessage(receipt, "&3(From " + staffUtils.getPlayerColor(sender) + sender.getName() + "&3) " + message);
		staffUtils.informativeMessage(receipt, "&3(To " + staffUtils.getPlayerColor(receipt) + receipt.getName() + "&3) " + message);
	}
}
